...
const AdminList = () => {
  ...
  const fetchAdmins = async (pageNum = page) => {
    try {
      // truyền role admin,manage dưới dạng chuỗi
      const res: any = await getUserList({ _page: pageNum, _limit: perPage, role: "admin,manage" });

      setAdmins(Array.isArray(res.data.data) ? res.data.data : []);
      setTotal(typeof res.data.result === "number" ? res.data.result : 0);
    } catch (error) {
      console.error("Lỗi khi lấy danh sách admin và manage:", error);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      console.error("Lỗi khi xóa admin/manager:", error);
    }
  };
  ...
  return (
    <div className="p-6 bg-white rounded-lg shadow-md">
      <h2 className="text-2xl font-semibold text-gray-800 mb-6">Quản lý Admin và Manager</h2>
            ...
            {admins.length === 0 ? (
              <tr>
                <td colSpan={9} className="text-center py-6 text-gray-500">
                  Không có admin hoặc manage nào
                </td>
              </tr>
            ) : (
