    ...
    <section>
      ...
      <div className="mb-6 space-y-1 max-w-xs">
        <div className="flex justify-between text-gray-700 text-sm">
          <span>Tổng phụ</span>
          <span>${cartData?.subtotal?.toFixed(2) || '0.00'}</span>
        </div>
        <div className="flex justify-between text-gray-700 text-sm">
          <span>Phí giao hàng</span>
          <span>${cartData?.shipping?.toFixed(2) || '0.00'}</span>
        </div>
        <div className="flex justify-between text-sm">
          <span>Phiếu giảm giá</span>
          <a href="#" className="text-teal-600 hover:underline">Apply Discount</a>
        </div>
        <div className="border-t border-gray-300 mt-4 pt-3 font-semibold text-gray-900 text-base">
          Tổng số tiền
          <span className="float-right">${cartData?.total?.toFixed(2) || '0.00'}</span>
        </div>
      </div>
        ...
        {cartData?.items?.map((item) => (
          <li key={item.id} className="flex items-center space-x-4 border border-gray-200 rounded-lg p-3 shadow-sm hover:shadow-md transition-shadow">
            ...
            <div className="flex-grow">
              ...
              <p className="font-bold text-teal-600">${item.price.toFixed(2)} x {item.quantity}</p>
            </div>
          </li>
        )) || (
